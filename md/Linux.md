...
| `cat myfile | grep '^s.*n$'`" means find a line ending with an "a". For example, this command searches the file myfile for lines starting with    an "s" and ending with an "n".$                    | means "the end of the line". So "a$ |                                                              |
...
## Commands Related to Runlevels (on Kubuntu at least)
...
When there isn't any error, the library usually does not set `errno` and returns the value it obtained from the kernel.
...
Other commands mentioned in the course were:
...
You can echo directly to `/proc`, `/dev`, or `/sys` to explicitly tell the kernel to interact with a device.

If you put the Linux kernel source code in the `/usr/src` directory (where an example absolute path is `/usr/src/linux-5.3.0/include/uapi/asm-generic`), we have:

```bash
$ ls -l unistd.h
-rw-r--r-- 1 kevin kevin 30377 Sep 15 14:19 unistd.h
$ wc -l unistd.h
903 unistd.h
$ grep -i read !$      # !$ represents the last token from most recent command
...
The unistd.h file contains information about system calls. In accordance with the C programming language, unistd.h is the name of the header file that provides access to the POSIX operating system API.
...
ERRORS
       ...
       EINVAL fd  is  attached  to an object which is unsuitable for reading; or the file was opened with the O_DIRECT flag, and either the address specified in buf, the value specified in count, or the file offset is not suitably
              aligned.
       ...
       EIO    I/O error.  This will happen for example when the process is in a background process group, tries to read from its controlling terminal, and either it is ignoring or blocking SIGTTIN or its process group is orphaned.
              It  may  also  occur when there is a low-level I/O error while reading from a disk or tape.  A further possible cause of EIO on networked filesystems is when an advisory lock had been taken out on the file descriptor
...
NOTES
       ...
       On NFS filesystems, reading small amounts of data will update the timestamp only the first time, subsequent calls may not do so.  This is caused by client side attribute caching, because most if not all  NFS  clients  leave
       st_atime  (last  file  access  time) updates to the server, and client side reads satisfied from the client's cache will not cause st_atime updates on the server as there are no server-side reads.  UNIX semantics can be ob‐
...
BUGS
       ...
       Among the APIs subsequently listed are read() and readv(2).  And among the effects that should be atomic across threads (and processes) are updates of the file offset.  However, on Linux before version 3.14,  this  was  not
       the  case:  if  two processes that share an open file description (see open(2)) perform a read() (or readv(2)) at the same time, then the I/O operations were not atomic with respect updating the file offset, with the result
       that the reads in the two processes might (incorrectly) overlap in the blocks of data that they obtained.  This problem was fixed in Linux 3.14.
...
Linux                                                                                                         2018-02-02                                                                                                       READ(2)
 Manual page read(2) line 26/81 (END) (press h for help or q to quit)

```

Enter `man syscalls` for more information on system calls.

printk() is the kernel's function for code to print messages. It is like C's `printf()`.

`dmesg` displays messages from the kernel that are currently stored in the RAM buffer, which is a ring buffer (aka circular buffer). A ring buffer is basically an array that wraps the write pointer around to the first element again whenever the write pointer reaches the end of an array, so that no data has to be removed from the array, but is instead overwritten. Note that the RAM buffer is reset every time you reboot.

`/var/log/messages` has kernel messages and more.

`tail -f /var/log/messages` can be handy.

`journalctl -t kernel` show messages for the specified syslog identifier, in this case the kernel syslog.

`journalctl -t kernel -f` Show only the most recent journal entries, and continuously print new entries as they are appended to
 the journal.

`dmesg | wc -l` displays the number of lines in the RAM buffer.

`dmesg | grep command` or `cat /proc/cmdline` displays the parameters passed to the kernel at the time it is started:

- absolute path of the boot image (the kernel)
- the Universal Unique Identifier (UUID) associated with the root partition (try `sudo blkid` for more clarity on this)
- stands for "read-only", meaning that the root filesystem is mounted as read-only upon startup.
...
```
BOOT_IMAGE=/boot/vmlinuz-5.13.0-44-generic root=UUID=7ebe4a1a-dbdb-4143-83dd-661d539ef6d8 ro quiet splash vt.handoff=7
```
...
## Can you determine, using strace, what system call is used to change the directory?

Yes, the `chdir` system call is used to change the current working directory in Linux. To determine this using `strace`, you can run a command with `strace` and observe the system calls it makes.

For example, let's use `strace` with the `cd` command:

```

$ strace cd /path/to/directory
```

When running this command, `strace` will trace and display the system calls made by the `cd` command. Among the system calls displayed, you should see the `chdir` system call being used to change the directory.

Note that `strace` attaches to the execution of a process, so it may not be directly used with the `cd` command alone since it's a shell built-in command. Instead, you can use it with other commands that involve changing directories, such as executing a script or a program that changes the directory.

## By looking at include/uapi/asm-generic/unistd.h, determine about how many system calls are defined in the kernel source?

1. Locate the kernel source code: Obtain the source code of the kernel you are interested in. This typically involves downloading the kernel source from the official source repository or using a package manager.
2. Navigate to the `unistd.h` file: Once you have the kernel source code, navigate to the appropriate directory where the `unistd.h` file is located. In many cases, it can be found in the `include/uapi/asm-generic` directory.
3. Open the `unistd.h` file: Use a text editor or a command-line tool to open the `unistd.h` file in order to inspect its contents.
4. Count the system call definitions: In the `unistd.h` file, look for function prototypes that start with `__SYSCALL_DEFINEx` macros (where `x` can be a number). Each of these macros represents a system call definition. Count the number of such macros to determine the total number of system calls defined in that file.

Remember that the `unistd.h` file may include other files or macros that define additional system calls or related constants. Consider exploring those dependencies as well to get a comprehensive count.

Please note that the exact steps can vary depending on the specific kernel source version and organization. It's recommended to refer to the documentation or specific guides related to the kernel source you are working with for more accurate instructions.

## Run a `sleep 100` with `&` (to put it in the background). What files does its process have open?

Hypothetically, if you were to run the `sleep` command with the `&` symbol to put it in the background, the process created by the `sleep` command would typically have certain standard files open. Here are the standard files that a background `sleep` process might have open:

1. Standard Input (stdin): The file descriptor (FD 0) representing the standard input file, which is typically connected to the terminal.
2. Standard Output (stdout): The file descriptor (FD 1) representing the standard output file, which is usually connected to the terminal.
3. Standard Error (stderr): The file descriptor (FD 2) representing the standard error file, also connected to the terminal.

When a process is executed in the background with the `&` symbol, it typically inherits these standard files from its parent process. However, since the process is running in the background and detached from the terminal, the standard input, output, and error might be redirected or closed by the shell or the terminal emulator.

It's important to note that the actual file descriptors and their states can vary depending on the specific environment and shell settings. To obtain the precise information about the files open by a running process, tools like `lsof` (List Open Files) or `proc` file system (e.g., examining the `/proc/<pid>/fd/` directory) can be used on Unix-like systems to inspect the open files for a specific process ID (PID).

## Does your system have an PCI Ethernet device?

To determine if your Linux system has a PCI Ethernet device, you can use various commands and tools available in the Linux environment. Here are a few approaches you can try:

1. Using `lspci`:
   - Open a terminal or command prompt.
   - Run the command `lspci` (list PCI devices).
   - Look for an entry that corresponds to an Ethernet controller or network interface card (NIC). It might be labeled as "Ethernet controller," "Network controller," or something similar.
   - The output should display information about the PCI Ethernet device(s) present in your system.
2. Using `ip` command:
   - Open a terminal or command prompt.
   - Run the command `ip link show`. This command shows the network interfaces on your system.
   - Look for an interface name starting with "en" or "eth" (e.g., `eth0`, `enp0s3`). These names often indicate Ethernet devices. The associated information will mention the type of interface, such as Ethernet, and provide additional details.
3. Using `ifconfig`:
   - Open a terminal or command prompt.
   - Run the command `ifconfig -a`. This command displays information about the network interfaces, including Ethernet devices.
   - Look for entries labeled as "Ethernet" or "Ethernet controller" in the output.

These commands should help you identify the presence of PCI Ethernet devices on your Linux system. Keep in mind that the specific command options and output may vary slightly depending on your Linux distribution and version.
